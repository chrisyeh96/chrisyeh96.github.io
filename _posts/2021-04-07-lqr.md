...
* For a matrix $$M \in \R^{n \times m}$$, $$M^\dagger$$ denotes its Moore-Penrose pseudoinverse ([Wikipedia](https://en.wikipedia.org/wiki/Moore%E2%80%93Penrose_inverse)).
...
- states: $$s_t \in \R^n$$
- actions: $$a_t \in \R^m$$
- random noise: $$w_t \in \R^n$$, $$\E[w_t] = \zero$$, each $$w_t$$ is independent
  - The only assumption on the random vector $$w_t$$ is that it has zero mean and finite covariance. No other assumptions are made about its distribution.
...
- In standard LQR, the minimum instantaneous cost is achieved by $$s, a = \zero$$, which is a fixed point in the linear system dynamics. Therefore, the optimization objective is essentially to find a series of actions that moves the state closer and closer to the origin.
...
given $$\delta_t \in \R$$, $$D_t \in \Sym^{n+m}$$, and $$d_t \in \R^{n+m}$$. We require $$D_t \succeq 0$$ for all $$t$$, $$R_t \succ 0$$ for all $$t < T$$, and $$d_{T,a} \in \range(R_T)$$. Setting $$S_t = \zero$$ and $$d_t = \zero$$ recovers the standard LQR cost function.
...
- $$(I - Q_t Q_t^\dagger) S_t = \zero$$ and $$R_t - S_t^\top Q_t^\dagger S_t \succeq 0$$,
- $$(I - R_t R_t^\dagger) S_t^\top = \zero$$ and $$Q_t - S_t R_t^\dagger S_t^\top \succeq 0$$.
...
**Lemma (minimum of affine quadratic function):** Consider the function $$c: \R^n \times \R^m \to \R$$ given by
...
$$\begin{aligned}
    \argmin_{a \in \R^m} c(s,a)
    &= \set{ -R^\dagger \left( S^\top s + \frac{1}{2} d_a \right) + y \ \middle| \ y \in \nullspace R } \\
    &= \set{ Ks + k + y \ \middle| \ y \in \nullspace R } \\
    &= \set{ Ks + k + (I - R^\dagger R) y \ \middle| \ y \in \R^m },
\end{aligned}$$

where $$K := -R^\dagger S^\top$$ and $$k := -\frac{1}{2} R^\dagger d_a$$. For simplicity, let's pick $$y = \zero \in \nullspace R$$, and set $$a^* := Ks+k$$. Then,
...
$$\begin{aligned}
    ...
    &= 2 S k + S R^\dagger R R^\dagger d_a - S R^\dagger d_a + d_s \\
...
For the inductive step, suppose that the hypothesis holds for some time step $$t+1$$. By the preceding lemma, the optimal value function at time $$t+1$$ is

$$
    v_{t+1}(s) = s^\top Z_{t+1} s + z_{t+1}^\top s + \text{const}
$$

where $$Z_{t+1} \succeq 0$$ and $$z_{t+1}$$ are functions of $$P_{t+1}$$ and $$p_{t+1}$$. The constant term is $$\text{const} = -\frac{1}{4} p_{t+1,a}^\top P_{t+1,a}^\dagger p_{t+1,a} + \beta_{t+1}$$.
...
where $$P = D + F^\top Z_{t+1} F$$ and $$p = d + F^\top z_{t+1}$$. The first equation for $$\E_w[v_{t+1}(s')]$$ uses the assumption that $$\E[w] = \zero$$. The second equality absorbs $$\E_w[w^\top Z_{t+1} w] = \tr(Z_{t+1} \Cov[w])$$ into $$\text{const}_w$$ because it is constant with respect to states and actions; here, we have implicitly used the assumption that $$\Cov[w]$$ is finite so that the cost function is not infinite. The constant term is therefore

$$
    \beta
    = \delta + \text{const}_w
    = \delta + \E_w[ w^\top Z_{t+1} w ] - \frac{1}{4} p_{t+1,a}^\top P_{t+1,a}^\dagger p_{t+1,a} + \beta_{t+1}.
$$
...
Additionally, it is common to expand out the backward recursion equations (which removes the need for explicitly constructing $$P$$ and $$p$$).
...
In particular, the expression for $$Z$$ is known as the **discrete-time Riccati equation**. Sometimes, this is called the _dynamic_ Riccati equation or the Riccati _difference_ equation to distinguish it from the _algebraic_ Riccati equation that arises in infinite-horizon LQR.

Finally, since $$q_t(s,a)$$ satisfies the inductive hypothesis for all $$t$$, the optimal action can be computed by applying the preceding lemma again. Specifically, at time $$t$$, the optimal action is
...
For the inductive hypothesis, suppose that $$z_{t+1} = \zero$$. The base case is satisfied with $$z_{T+1} = \zero$$. Then the recursion vectors all become 0:

$$\begin{aligned}
    p &= d + F^\top z_{t+1} = \zero + F^\top \zero = \zero \\
    k &= -\frac{1}{2} P_a^\dagger p_a = -\frac{1}{2} P_a^\dagger \zero = \zero \\
    z &= 2 P_{sa} k + p_s = 2 P_{sa} \zero + \zero = \zero
\end{aligned}$$
...
$$\begin{aligned}
    ...
    Z &= Q + A^\top Z_{t+1} A - (A^\top Z_{t+1} B) (R + B^\top Z_{t+1} B)^\dagger (A^\top Z_{t+1} B)^\top
\end{aligned}$$
...
so $$\beta = \tr(\Sigma Z_{t+1}) + \beta_{t+1}$$. Applying the lemma on the minimum of affine quadratic functions to $$q_t(s,a)$$, the value function at each time step is precisely
...
In addition, $$q_{t-1}$$ and $$v_{t-1}$$ have the same form as $$q_t$$ and $$v_t$$, and I derived recursive formulas for $$P_t, p_t, Z_t, z_t$$ in terms of the given cost matrices for all $$t = T, \dotsc, 0$$. In particular, the equation for $$Z_t$$ is known as the **Riccati equation**.

Finally, given $$P_t, p_t$$, I showed how to compute $$K_t, k_t$$ such that the optimal action from any state $$s_t$$ is

$$ a_t = K_t s_t + k_t. $$

The set of controllers $$\{(K_t, k_t)\}_{t=0}^T$$ form the **linear quadratic regulator** (LQR). Importantly, the LQR is optimal from any state $$s$$ and does not depend on the noise distribution for the $$w_t$$. The optimal policy is always the same, as long as $$w_t$$ has zero mean and finite covariance!
...
with an additional constant vector $$C \in \R^n$$. This can actually be re-written as a standard linear dynamical system:
...
With state and input constraints, the lemma on the minimum of affine quadratic functions no longer applies, so the recursion equations no longer hold.
...
- See [Borrelli et al. (2016) textbook](http://www.mpc.berkeley.edu/mpc-course-material) (Chapters 11.3.3 and 17.6) for more details on the dynamic programming approach.
...
$$
    J = \E\left[ \sum_{t=0}^T s_t^\top Q_t s_t + a_t^\top R_t a_t \right]
$$
...
Even though I only provided a derivation for the finite-horizon discrete-time setting, the results are similar for continuous-time and infinite-horizon settings. Essentially, the sum in the cost function is replaced by an integral, and the discrete-time algebraic Riccati equation is replaced by a continuous-time Riccati differential equation.
...
- *Predictive Control for Linear and Hybrid Systems* by Borrelli et al. (2016). Available [here](https://sites.google.com/berkeley.edu/mpc-lab/mpc-course-material).
