...
$$
    x^T M x = x^T \left[ \frac{1}{2} (M + M^\top) \right] x.
$$

In other words, $$x^T M x$$ can always be replaced with an equivalent expression $$x^T \hat{M} x$$ where $$\hat{M} = \frac{1}{2} (M + M^\top)$$ is symmetric.
...
$$
    x^T \left[ \frac{1}{2} (M + M^\top) \right] x
    = \frac{1}{2} (x^T M x + x^T M^T x)
    = x^T M x.
$$
